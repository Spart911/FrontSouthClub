...
1. Отправляет данные заказа в формате JSON через POST `/api/v1/orders/`
2. Получает `order_id` и `confirmation_token`
3. Запрашивает полные данные заказа через GET `/api/v1/orders/{order_id}`
4. Отображает номер заказа (если доступен) или использует order_id как fallback
5. Инициализирует виджет YooKassa с `confirmation_token`
6. Очищает сообщение загрузки при успешной инициализации виджета

### Отправляемые данные заказа:
```json
{
...
### Примечания по работе платежного виджета:

- **SberPay URL scheme**: Ошибка `"Failed to launch 'sberpay://...' because the scheme does not have a registered handler"` является нормальной. Браузер не может открыть мобильное приложение SberPay - это ожидаемое поведение.
- **Номер заказа**: Если API не возвращает `order_number`, используется `order_id` как резервный вариант.
- **Загрузка виджета**: Сообщение "Загрузка формы оплаты..." автоматически очищается при успешной инициализации виджета YooKassa.
- **Allow attribute warning**: Предупреждение `"Allow attribute will take precedence over 'allowpaymentrequest'"` является нормальным. Это информационное сообщение браузера о том, что для iframe платежной формы используется современный атрибут `allow` вместо устаревшего `allowpaymentrequest`. Не влияет на работу оплаты.
